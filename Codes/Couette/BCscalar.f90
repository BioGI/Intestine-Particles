...
ELSE 			     				! Calculates phiWall for flux BC (eq. 28 in paper)				
   phiWall= ((phiTemp(i,j,k)*(1.0+q)*(1.0+q)/(1.0+2.0*q)) - (phiTemp(ip1,j,k)*q*q/(1.0+2.0*q)) - (q*(1+q)/(1+2.0*q))* (coeffConst/coeffGrad)) &
...
CALL Equilibrium_LOCAL(m,rhoAstar,ub,vb,wb,feq_Astar)    		! Equibrium distribution function in the mth direction
