...
REAL(dbl)    :: mass1,mass2,mass3,mass4							! mass sums
...
IF(myid .EQ. master) THEN
  ...
  DO nn=1,numLines
    ...
    DO n=1,NumSubsTotal
      ...
      mass3 = mass3 + MassData(nn,3,n)
      mass4 = mass4 + MassData(nn,4,n)

    END DO

    i = (iter0-1_lng) + nn

    WRITE(2459,'(6E15.5)') REAL(i/(nt/nPers)),timeread(i),mass1, mass2,mass3,mass4	
...
REAL(dbl)		:: phi1,phi2,phi3,phi4,phi5, phi6,phi7,phi8	! scalar sums
...
REAL(dbl)		:: phiInitial(iter0:nt)						! Initial Scalar present in the domain
REAL(dbl), ALLOCATABLE		:: timeread(:)						! time
...
IF(myid .EQ. master) THEN
  ...
  ALLOCATE(ScalarData(numLines,8,NumSubsTotal))
  ALLOCATE(timeread(numLines))
...
!      WRITE(6678,*) nn, i
  ...
  WRITE(2473,'(A115)') '#VARIABLES = "period","time","phiA", "phiAS", "phiAV", "phiT-phiD", "phiD", "phA+phiD", "phiAverage","phiInitial"'
  ...
  DO nn=1,numLines
    ...
    DO n=1,NumSubsTotal
      ...
      phi8 = phi8 + ScalarData(nn,8,n)
    END DO

    i = (iter0-1_lng) + nn

    WRITE(2473,'(10E25.15)') REAL(i/(nt/nPers)),timeread(i),phi1, phi6, phi7, phi2, phi3, phi4, phi5/NumSubsTotal,phi8				! write to combined output file

    phiInitial(i) = phi8 																					! store initial total scalar
